...
canvas.width = 1000; // muss exakt dem CSS-Wert entsprechen
canvas.height = 500;
let counter = 0;
...
const boxDefinitions = [
  { color: "yellow", border: "black", x: 200, y: 20 },
  { color: "pink",   border: "black", x: 700, y: 20 },
  { color: "orange", border: "black", x: 450, y: 150 },
  { color: "blue",   border: "black", x: 200, y: 250 },
  { color: "green",  border: "black", x: 700, y: 250 }
];
let showArrow = true;
let arrowY = 0;
...
function checkMode() {
  ...
  document.querySelectorAll('#mode-wrapper p').forEach(p => {
    if (p.id === currentMode) {
...
if (currentMode === "mode-3") rekordText = `<p>Dein Rekord: <strong>${rekord3}</strong> 🏆</p>`;
  ...
  const content = `
    <div>
      <h1 style="margin-bottom: 1em;">🎨 Schade du bist leider raus! 😪</h1>
